...
class RobotDataManager(Node):
        ...
        for i in range(self.num_envs):
            if (self.num_envs == 1):
                ...
                self.quaternion.append(
                    self.create_publisher(Quaternion, "unitree_go2/quaternion", 10)
                )
                self.joint_pos.append(
                    self.create_publisher(JointState, "unitree_go2/lowstate", 10)
                )
                #self.cmd_vel_sub.append(                                    # [go2_ctrl.py]                                                    
                #    self.create_subscription(Twist, "unitree_go2/cmd_vel",  # [go2_ctrl.py] S'abonne au topic "/unitree_go2/cmd_vel"           
                #    lambda msg: self.cmd_vel_callback(msg, 0), 10)          # [go2_ctrl.py] Appelle cmd_vel_callback(msg, 0) à chaque msg recu 
                #)
    ...
    def pub_ros2_data_callback(self):                                                                # [go2_env.py]                                          
        robot_data = self.env.unwrapped.scene["unitree_go2"].data                                    # [go2_env.py]                                          
        for i in range(self.num_envs):                                                               # [go2_env.py]                                          
            self.publish_odom(robot_data.root_state_w[i, :3],                                        # [go2_env.py]                                          
                              robot_data.root_state_w[i, 3:7],                                       # [go2_env.py]                                          
                              robot_data.root_lin_vel_b[i],                                          # [go2_env.py]                                          
                              robot_data.root_ang_vel_b[i],                                          # [go2_env.py]                                          
                              i)                                                                     # [go2_env.py]                                          
            self.publish_pose(robot_data.root_state_w[i, :3],                                        # [go2_env.py]                                          
                              robot_data.root_state_w[i, 3:7], i)                                    # [go2_env.py]                                          
                                                                                                    
    def pub_lidar_data_callback(self):
        for i in range(self.num_envs):
            self.publish_lidar_data(self.lidar_annotators[i].get_data()["data"].reshape(-1, 3), i)

    def pub_ros2_data(self):                                                                         # [go2_env.py] Fonction qui publie odom, pose et lidar  
        pub_odom_pose = False                                                                        # [go2_env.py] Elle est appelée par la boucle principale
        pub_lidar = False                                                                            # [go2_env.py] de simulation pour determiner les obs    
        dt_odom_pose = time.time() - self.odom_pose_pub_time                                         # [go2_env.py]                                          
        dt_lidar = time.time() - self.lidar_pub_time                                                 # [go2_env.py]                                          
        if (dt_odom_pose >= 1./self.odom_pose_freq):                                                 # [go2_env.py]                                          
            pub_odom_pose = True                                                                     # [go2_env.py]                                          
                                                                                                     # [go2_env.py]                                          
        if (dt_lidar >= 1./self.lidar_freq):                                                         # [go2_env.py]                                          
            pub_lidar = True                                                                         # [go2_env.py]                                          
                                                                                                     # [go2_env.py]                                          
        if (pub_odom_pose):                                                                          # [go2_env.py]                                          
            self.odom_pose_pub_time = time.time()                                                    # [go2_env.py]                                          
            robot_data = self.env.unwrapped.scene["unitree_go2"].data                                # [go2_env.py]                                          
            for i in range(self.num_envs):                                                           # [go2_env.py]                                          
                self.publish_odom(robot_data.root_state_w[i, :3],                                    # [go2_env.py]                                          
                                robot_data.root_state_w[i, 3:7],                                     # [go2_env.py]                                          
                ...
                self.publish_pose(robot_data.root_state_w[i, :3],                                    # [go2_env.py]                                          
                                robot_data.root_state_w[i, 3:7], i)                                  # [go2_env.py]                                          
        if (self.cfg.sensor.enable_lidar):                                                           # [go2_env.py]                                          
            if (pub_lidar):                                                                          # [go2_env.py]                                          
                self.lidar_pub_time = time.time()                                                    # [go2_env.py]                                          
                for i in range(self.num_envs):                                                       # [go2_env.py]                                          
                    self.publish_lidar_data(self.lidar_annotators[i].get_data()["data"].reshape(-1, 3), i)
        
        
        base_rot = robot_data.root_state_w[0, 3:7]  # [go2_env.py]  Quaternion                              
        quat_msg = Quaternion()                     # [go2_env.py]                                          
        quat_msg.x = base_rot[1].item()             # [go2_env.py]                                          
        quat_msg.y = base_rot[2].item()             # [go2_env.py]                                          
        quat_msg.z = base_rot[3].item()             # [go2_env.py]                                          
        quat_msg.w = base_rot[0].item()             # [go2_env.py]                                          
        self.quaternion[0].publish(quat_msg)        # [go2_env.py]                                          

        
        joint_pos = robot_data.joint_pos[0].cpu().tolist()                                                # [go2_env.py]  Jointstate                              
        joint_msg = JointState()                                                                          # [go2_env.py]                                          
        #joint_msg.name = [f"joint{i}" for i in range(len(joint_pos))]  # Ex: ["joint0", ..., "joint11"]  # [go2_env.py]                                          
        joint_msg.position = joint_pos                                 # Les 12 positions articulaires    # [go2_env.py]                                          
            
        joint_vel = robot_data.joint_vel[0].cpu().tolist()            # [go2_env.py]                      
        joint_msg.velocity = joint_vel                                # [go2_env.py]                      
        joint_msg.header.stamp = self.get_clock().now().to_msg()      # [go2_env.py]                      

        self.joint_pos[0].publish(joint_msg) 

    def cmd_vel_callback(self, msg, env_idx):                    # [go2_ctrl.py]                                         
        go2_ctrl.base_vel_cmd_input[env_idx][0] = msg.linear.x   # [go2_ctrl.py] Modifie la variable base_vel_cmd_input  
    ...
    def odom_callback(self, msg, env_idx):                                    # [go2_env.py] Modifie la variable base_lin_vel_input
        from go2 import go2_env                                               # [go2_env.py]                                       
        if go2_env.base_lin_vel is not None:                                  # [go2_env.py]                                       
            ...
            print(f"[ROS2] /odom → base_lin_vel[{env_idx}] = "                # [go2_env.py]                                       
                f"[{msg.twist.twist.linear.x:.2f}, "                          # [go2_env.py]                                       
                f"{msg.twist.twist.linear.y:.2f}, "                           # [go2_env.py]                                       
                f"{msg.twist.twist.linear.z:.2f}]")                           # [go2_env.py]                                       
